...
namespace functions_and_subprograms2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            double.TryParse(userInput1, out double inputnumber1);
            ...
            // Parse user input into integer. If user 
            double.TryParse(userInput2, out double inputnumber2);

            // Call function LesserNumber using user input as parameters
            Console.WriteLine("Antamistasi numeroista {0} oli pienempi", LesserNumber(inputnumber1, inputnumber2));

            // Wait for any key to be pressed before exiting
            Console.WriteLine("Press any key to continue...");
            Console.ReadKey();

            double LesserNumber(double number1, double number2)
            {
